...
public class ClienteDao {
    ...
    public List<ModeloCliente> list(){
        String sql = "SELECT * FROM "+ dbuser +"CLIENTES JOIN TIPOCLIENTES USING(TIPOCLIENTEID) WHERE NIT != 0";
    ...
    public void save(ModeloCliente nuevo) {
        ...
        insertActor.withTableName("clientes").usingColumns("nombre","nit","email","telefono","patente","tipoclienteid", "vencimiento");
    ...
    public ModeloCliente get(int nit) {
        try{
		String sql = "SELECT * FROM "+ dbuser +"CLIENTES JOIN TIPOCLIENTES USING(TIPOCLIENTEID) WHERE NIT = ?";
		Object[] args = {nit};
